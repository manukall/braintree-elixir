defmodule Braintree.HTTP do
  ...
  * public_key - Braintree public key
  ...
  def build_url(path, opts) do
    environment = opts[:environment] || Braintree.get_env(:environment, :sandbox)
    merchant_id = opts[:merchant_id] || Braintree.get_env(:merchant_id)
  ...
  def build_headers(opts) do
    public  = opts[:public_key] || Braintree.get_env(:public_key)
    private = opts[:private_key] || Braintree.get_env(:private_key)
